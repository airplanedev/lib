...
func node(root string, options KindOptions, buildArgs []string) (string, error) {
	...
	return applyTemplate(heredoc.Doc(`
		...
		{{if .IsDurable}}
		# In durable case, the package.json uses modules (to run the workflow bundler),
		# but the output generated by esbuild does not. Force node to use commonjs imports
		# by changing the shim name.
		RUN mv /airplane/.airplane/dist/shim.js /airplane/.airplane/dist/shim.cjs
		ENTRYPOINT ["node", "/airplane/.airplane/dist/shim.cjs"]
		{{else}}
...
func GenShimPackageJSON(pathPackageJSON string, isDurable bool) ([]byte, error) {
	...
	if isDurable {
		...
		// This is needed for bundle to work correctly.
		pjson.Type = "module"
	}
